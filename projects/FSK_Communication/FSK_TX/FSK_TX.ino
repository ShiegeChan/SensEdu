...
const uint16_t sine_lut_size_0 = 132; // sine wave size bit '0' 32000 Hz
...
0x000, 0x000, 0x027, 0x09C, 0x159, 0x258, 0x38E, 0x4F0, 
0x670, 0x800, 0x98F, 0xB0F, 0xC71, 0xDA7, 0xEA6, 0xF63, 
0xFD8, 0xFFF, 0xFD8, 0xF63, 0xEA6, 0xDA7, 0xC71, 0xB0F, 
0x98F, 0x800, 0x670, 0x4F0, 0x38E, 0x258, 0x159, 0x09C, 
0x027, 0x000, 0x000, 0x027, 0x09C, 0x159, 0x258, 0x38E, 0x4F0, 0x670, 
0x800, 0x98F, 0xB0F, 0xC71, 0xDA7, 0xEA6, 0xF63, 0xFD8, 
0xFFF, 0xFD8, 0xF63, 0xEA6, 0xDA7, 0xC71, 0xB0F, 0x98F, 
0x800, 0x670, 0x4F0, 0x38E, 0x258, 0x159, 0x09C, 0x027, 
0x000, 0x000, 0x027, 0x09C, 0x159, 0x258, 0x38E, 0x4F0, 
...
const uint16_t sine_lut_size_1 = 124; // sine wave size bit '1' 34000 Hz
...
0x587, 0x400, 0x2A5, 0x187, 0x0B1, 0x02D, 0x000, 0x000, 
0x02D, 0x0B1, 0x187, 0x2A5, 0x400, 0x587, 0x729, 0x8D6, 
0xA78, 0xBFF, 0xD5A, 0xE78, 0xF4E, 0xFD2, 0xFFF, 0xFD2, 
0xF4E, 0xE78, 0xD5A, 0xBFF, 0xA78, 0x8D6, 0x729, 0x587, 
0x400, 0x2A5, 0x187, 0x0B1, 0x02D, 0x000, 
0x000, 0x02D, 0x0B1, 0x187, 0x2A5, 0x400, 0x587, 0x729, 
0x8D6, 0xA78, 0xBFF, 0xD5A, 0xE78, 0xF4E, 0xFD2, 0xFFF, 
0xFD2, 0xF4E, 0xE78, 0xD5A, 0xBFF, 0xA78, 0x8D6, 0x729, 
0x587, 0x400, 0x2A5, 0x187, 0x0B1, 0x02D, 0x000, 0x000, 
...
const uint16_t MAX_MESSAGE_LENGTH = 32; // Maximum allowed by arduino
...
void buildMessageLUT (uint8_t* data, uint8_t num_bytes) {
    int sync_signal_size = 0;
    uint16_t position = sync_signal_size;

    // Clear the entire LUT first (fill with DC level, e.g., 0x800)
    for (size_t i = 0; i < MAX_LUT_SIZE + sync_signal_size; i++) {
        lut[i] = 0x800;  // Mid-level (silence)
    }

    // Create sync signal
    // for (size_t i = 0; i < sync_signal_size; i++) {
    //     lut[i] = 0xFFF;  // High-level
    // }
