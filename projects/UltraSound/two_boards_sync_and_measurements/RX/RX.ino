...
uint8_t pin_TXfinished = 7; // digital pin for syncronization
uint8_t pin_RXfinished = 8; // digital pin for syncronization
...
void setup() {
  pinMode(pin_TXfinished, INPUT);
  pinMode(pin_RXfinished, OUTPUT);
...
void loop() {
    ...
    digitalWrite(pin_RXfinished, 0);
    while(digitalRead(pin_TXfinished) == 0) {}; // wait for the sync_pin to signal that the board can start with acquisition
