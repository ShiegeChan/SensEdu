...
bool try_save_pressure_meas(float pressure_hpa) {
...
bool try_get_pressure_trend(float* trend_hpa_per_hour) {
    if (pressure_history_count < PRESSURE_TREND_SAMPLES) return false;
...
// Prints the current pressure trend and how long until the next sample is captured.
void report_pressure_trend(void) {
    float trend = 0.0f;
    if (try_get_pressure_trend(&trend)) {
        Serial.print("Pressure Trend: ");
        Serial.print(trend, 2);
        Serial.println(" hPa/h");
    } else {
        Serial.print("Pressure Trend: collecting data (");
        Serial.print(pressure_history_count);
        Serial.print("/");
        Serial.print(PRESSURE_TREND_SAMPLES);
        Serial.println(" samples)");
    }

    Serial.print("Next pressure sample in: ");
    uint32_t remaining_min = ms_until_next_pressure_meas() / 60000UL;
