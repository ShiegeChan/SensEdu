...
%   Shared by the live (EMG_BioInputs) and recorder (EMG_Raw_Recorder) scripts.
%   Returns is_recorded = false (and data = 0) when less than one full
%   half-buffer is queued, so the caller can simply skip the iteration. When
%   data is available it reads as many whole half-buffers as are queued and
%   returns them as a uint16 row vector (still channel-interleaved).
