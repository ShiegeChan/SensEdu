...
### Chirp Signal
{: .text-yellow-100}
FMCW radar involves the continuous transmission and reception of a frequency modulated signal also known as chirp. We will call the transmitted signal <span style="color:#57c2da !important">$$T_x$$</span>{: .text-blue-000} and we are using a sawtooth modulation. The sawtooth modulation linearly sweeps a bandwidth $$B$$ over the chirp period $$T_c$$.
...
We know how to compute the distance, that's great! But how do we extract $$f_b$$? We talk about this in the next section.
...
{: .IMPORTANT}
Distance resolution is only dependent on the signal's bandwidth !

For ultrasonic FMCW ranging, the maximum range $$d_{max}$$ is heavily limited by $$T_c$$. The maximum range $$d_{max}$$ is defined by
...
{: .IMPORTANT}
For ultrasonic FMCW ranging, the max range bottleneck is $$T_c$$. For regular FMCW radars, it's usually the ADC sampling frequency.


## Radar Design
{: .text-yellow-300}

The following block diagram illustrates the architecture of the FMCW ranging algorithm:

<img src="{{site.baseurl}}/assets/images/Radar.png" alt="drawing" width="800"/>
{: .text-center}

FMCW Ranging Algorithm Block Diagram
{: .text-center}
